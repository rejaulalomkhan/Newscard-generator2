...
const SYSTEM_FIELDS = ['image', 'imageScale', 'imageX', 'imageY', 'imageObjectFit', 'logo', 'adImage', 'font', 'accentColor', 'bgColor', 'textColor'];
...
function init() {
    ...
    elements.imageScale.addEventListener('input', (e) => { state.imageScale = parseFloat(e.target.value); saveState(); render(); });
...
function render() {
    ...
    const mainImg = elements.renderTarget.querySelector('[data-main-image="true"]');
    if (mainImg) {
...
function exportSettings() {
    ...
    link.download = `photocard-settings-${Date.now()}.json`;
    link.click();
...
function importSettings(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            // Merge imported settings with current state
            state = { ...state, ...imported };
            
            // Update all UI elements
            elements.templateSelect.value = state.templateId;
            if (state.font) fontChoices.setChoiceByValue(state.font);
            elements.accentColor.value = state.accentColor;
            elements.bgColor.value = state.bgColor;
            elements.textColor.value = state.textColor;
            if (state.image && !state.image.startsWith('data:')) elements.imageUrl.value = state.image;
...
async function fetchFromUrl() {
    ...
    if (!url) {
        elements.fetchStatus.textContent = 'Please enter a URL';
    ...
    elements.fetchStatus.textContent = 'Loading...';
